...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok left right = between (charTok left) (charTok right)
...
hex ::
  Parser Char
hex = match =<< replicateA 4 (satisfy isHexDigit)
  where
    match cs = case readHex cs of
...
eof = P assertEmpty
  where
    assertEmpty Nil = Result Nil ()
    assertEmpty cs = ExpectedEof cs
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll = satisfy . allSatisfy
    where
      allSatisfy = foldRight (lift2 (&&)) (const True)
      -- allSatisfy = foldRight (\newC2B curC2B c -> curC2B c && newC2B c) (const True) c2bs
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny = satisfy . anySatisfy
    where
      anySatisfy = foldRight (lift2 (||)) (const False)
      -- anySatisfy = foldRight (\newC2B curC2B c -> curC2B c || newC2B c) (const False) c2bs
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma c c' pa = betweenCharTok c c' (sepby pa (is ','))
