...
jsonString ::
  Parser Chars
jsonString = between (is '"') (is '"') (list moreStuff)
  where
    moreStuff = noneof ('"' :. '\\' :. Nil) ||| special ||| hexString
    special = is '\\' *> (fromSpecialCharacter <$> specialCharParser)
    hexString = string ('\\' :. 'u' :. Nil) *> hex

specialCharParser :: Parser SpecialCharacter
specialCharParser = matchSpecial =<< character
  where
    matchSpecial c = case toSpecialCharacter c of
...
jsonValue ::
  Parser JsonValue
jsonValue = spaces *> mkValue <* spaces
  where
    mkValue =
      JsonNull <$ jsonNull |||
      JsonTrue <$ jsonTrue |||
