...
character ::
  Parser Char
character = P errorIfEmpty
  where
    errorIfEmpty Nil = UnexpectedEof
    errorIfEmpty (c :. cs) = Result cs c
...
(|||) ::
  ...
  -> Parser a
  -> Parser a
(|||) (P i2ra) (P i2ra') = P $ \i -> i2ra i `or'` i2ra' i
  where
    or' ra ra'
      | isErrorResult ra = ra'
      | otherwise = ra
...
ageParser ::
  Parser Int
ageParser = match =<< list1 digit
  where
    match k = case read k of
...
surnameParser ::
  Parser Chars
surnameParser = combine <$> upper <*> thisMany 5 lower <*> list lower
  where
    combine c cs cs' = c :. cs ++ cs'
...
phoneParser ::
  Parser Chars
phoneParser = combine <$> digit <*> phoneBodyParser <*> (Nil <$ is '#')
  where
    combine c cs cs' = c :. cs ++ cs'
