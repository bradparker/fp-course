...
a ||| b = P $ \input ->
  case parse a input of
    Result input' res -> Result input' res
    _ -> parse b input
...
instance Monad Parser where
  ...
  aToPb =<< parser =
    P $ \input ->
      onResult (parse parser input) $ \input' a ->
        parse (aToPb a) input'
...
instance Applicative Parser where
  ...
  pAToB <*> pa =
    P $ \input ->
      onResult (parse pAToB input) $ \input' aToB ->
        parse (aToB <$> pa) input'
...
sequenceParser ::
  ...
  -> Parser (List a)
sequenceParser =
  foldRight (\p acc -> (:.) <$> p <*> acc) (pure Nil)
