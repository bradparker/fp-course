...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (<$>) f sa = StateT (\s -> first f <$> runStateT sa s)
...
distinct' ::
  ...
  List a
  -> List a
distinct' l = eval' (filtering isNotRepeat' l) S.empty
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF l = evalT (filtering isNotRepeatAndNotOver100 l) S.empty

when :: Applicative f => Bool -> f() -> f()
when a f = bool (pure ()) f a  

isNotRepeatAndNotOver100 :: (Ord a, Num a) => a -> StateT (S.Set a) Optional Bool
isNotRepeatAndNotOver100 a = do
    when (a >= 100) $ StateT $ const Empty
    s <- getT
    putT $ S.insert a s
    pure $ S.notMember a s
...
instance Monad f => Applicative (OptionalT f) where
  ...
  (<*>) optTf optTa = OptionalT $ run =<< runOptionalT optTf
    where
    --run :: Optional (a -> b) -> f (Optional b)
      run = onFull foo
    --foo :: (a -> b) -> f (Optional b)
      foo f = do
        optA <- runOptionalT optTa
        pure $ f <$> optA
...
instance Monad f => Monad (OptionalT f) where
  ...
  (=<<) f optTa = OptionalT $ foo =<< runOptionalT optTa
    where
    --foo :: Optional a -> f (Optional b)
      foo = onFull $ runOptionalT . f
...
distinctG l = runOptionalT $ evalT (filtering runDistinctG l) S.empty

runDistinctG :: (Integral a, Show a) => a -> StateT (S.Set a) (OptionalT (Logger Chars)) Bool
runDistinctG num = do
  when (num > 100) $ StateT $ const $ OptionalT $ log1 ("aborting > 100: " ++ show' num) Empty
  set <- getT
  putT $ S.insert num set
  let isMember = S.notMember num set
  if odd num
    then pure isMember
    else StateT $ OptionalT . log1 ("even number: " ++ show' num) . Full . (,) isMember
