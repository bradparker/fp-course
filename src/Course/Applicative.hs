...
instance Applicative ((->) t) where
  ...
  (<*>) ::
    ...
    -> (t -> a)
    -> t -> b
  (<*>) tAndAToB tToA t = tAndAToB t (tToA t)
...
sequence (fa :. fas) = pure (:.) <*> fa <*> sequence fas
...
replicateA ::
  ...
  -> f a
  -> f (List a)
replicateA i fa = sequence $ take i $ repeat fa
...
filtering f (a :. as) = (\b tail -> if b then a :. tail else tail) <$> f a <*> filtering f as
