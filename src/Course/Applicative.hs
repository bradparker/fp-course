...
lift4 f aa ab ac = (lift3 f aa ab ac <*>)
...
sequence = foldRight (\a -> ((:.) <$> a <*>)) (pure Nil)
...
filtering f as = map snd . filter fst . (`zip` as) <$> sequence (f <$> as)
